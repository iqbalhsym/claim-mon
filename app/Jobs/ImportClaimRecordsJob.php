    ...
    public function handle(): void
    {
        ...
        \App\Services\FastXlsxReader::readRows($this->filePath, function(array $cells, int $rowNumber) use (
            ...
            $batch[] = [
                ...
                'guarantor'                     => (string)($rawData['GUARANTOR'] ?? ''),
                'total_guarantee'               => (float)($rawData['TOTAL GUARANTEE'] ?? 0),
                'total_payment'                 => (float)($rawData['TOTAL PAYMENT'] ?? 0),
                'total_discount_per_item'       => (float)($rawData['TOTAL DISCOUNT PER ITEM'] ?? 0),
                'total_invoice_before_discount' => (float)($rawData['TOTAL INVOICE BEFORE DISCOUNT'] ?? 0),
                'hospital_guarantee'            => (float)($rawData['HOSPITAL GUARANTEE'] ?? 0),
                'total_must_be_paid'            => (float)($rawData['TOTAL MUST BE PAID'] ?? 0),
                'hospital_fee'                  => (float)($rawData['HOSPITAL FEE'] ?? 0),
                'doctor_guarantee'              => (float)($rawData['DOCTOR GUARANTEE'] ?? 0),
                'doctor_fee'                    => (float)($rawData['DOCTOR FEE'] ?? 0),
